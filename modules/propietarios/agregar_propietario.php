  ...
  <body>
    ...
    <header class="bg-primary text-white py-3 shadow-sm">
      <div class="container d-flex justify-content-between align-items-center">
          <a href="../../propietario.php" class="btn btn-outline-light d-flex align-items-center">
    ...
    <div class="container mt-5 mb-5">
      ...
      <div class="card mx-auto rounded" style="max-width: 600px;">
          ...
          <form id="miFormulario" action="" method="POST" novalidate>
            ...
            <button type="submit" class="btn btn-primary w-100">Agregar Propietario</button>
...
function agregarPropietario() {
    ...
    if (empty($nombre) || empty($direccion) || empty($telefono) || empty($correo)) {
        ...
        $sql = "INSERT INTO Propietario (nombre, direccion, telefono, correo) VALUES (?, ?, ?, ?)";

        if ($stmt = $conexion->prepare($sql)) {
            $stmt->bind_param("ssss", $nombre, $direccion, $telefono, $correo);

            if ($stmt->execute()) {
                echo '<script>alert("Propietario agregado correctamente."); window.location.href = "../../propietario.php";</script>';
            } else {
                echo "Error al agregar el Propietario: " . $stmt->error;
            }
